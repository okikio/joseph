...
export default class _event {
    ...
    // Alias for the `on` method
    get add() { return this.on; }
    get bind() { return this.on; }

    // Alias for the `off` method
    get remove() { return this.off; }
    get unbind() { return this.off; }

    // Alias for the `emit` method
    get fire() { return this.emit; }
    get trigger() { return this.emit; }

    // Alias for the `listeners` method
    get callbacks() { return this.listeners; }

    static nativeEvents = `ready load blur focus focusin focusout resize click scroll dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup contextmenu`.replace(/\s+/g, " ").split(" ");
