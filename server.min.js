  ...
  app.decorateReply('partial', function (filePath) {
    ...
    if (key in app.cache) {
      ...
      createReadStream(file).on("data", function (val) {
        ...
        dom = _toConsumableArray(fastHtmlParser.parse(data).querySelectorAll(partialSel));
        app.cache[key] = dom.filter(function (v) {
          return v.outerHTML;
        }).join("");
        res.type("text/html").send(app.cache[key]);
      });
  ...
  app.decorateReply('html', function (filePath) {
    ...
    if (key in app.cache) {
      ...
      createReadStream(file).on("data", function (val) {
        ...
        _this.type("text/html").send(data);
      });
    }
...
var _assets = fastifyPlugin(function (app, opts, next) {
  ...
  app.get("/assets/*", function (req, res) {
    ...
    if (key in app.cache) {
      ...
      var _url, height, width, quality, effect, crop, imgURLConfig;
