...
let _constrain = (v, a, b) => v > b ? b : v < a ? a : v; // Re-maps a number from one range to another. Numbers outside the range are not clamped to 0 and 1, because out-of-range values are often intentional and useful.

let _map = (e, t, n, r, i) => r + (i - r) * ((e - t) / (n - t)); // Log values
...
let map = (_el, fn) => {
  _el = el(_el);
  return el(_map$1(_el, (el, i) => fn.call(el, el, i)));
...
try {
  ...
  console.warn("Passive Events aren't supported in this browser, performance may suffer.");
...
on(window, 'scroll', () => {
  ...
  _images.forEach(data => {
    ...
    if (dist >= -60 && dist <= height - 60) {
      ...
      style(overlay, {
        opacity: _map(value, 0, 0.75, 0.15, 0.55)
      });
      style(load_img, {
